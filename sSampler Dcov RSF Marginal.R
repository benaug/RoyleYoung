sSamplerDcovRSF <- nimbleFunction(
  # name = 'sampler_RW',
...
sSamplerDcovRSF.tel <- nimbleFunction(
  # name = 'sampler_RW',
  contains = sampler_BASE,
  setup = function(model, mvSaved, target, control) {
    i <- control$i
    xlim <- control$xlim
    ylim <- control$ylim
    calcNodes.s.tel <- control$calcNodes.s.tel
    ## control list extraction
    # logScale            <- extractControlElement(control, 'log',                 FALSE)
    # reflective          <- extractControlElement(control, 'reflective',          FALSE)
    adaptive            <- extractControlElement(control, 'adaptive',            TRUE)
  ...
  run = function() {
    s.cand <- c(rnorm(1,model$s.tel[i,1],scale), rnorm(1,model$s.tel[i,2],scale))
    inbox <- s.cand[1]< xlim[2] & s.cand[1]> xlim[1] & s.cand[2] < ylim[2] & s.cand[2] > ylim[1]
    if(inbox){
      model_lp_initial <- model$getLogProb(calcNodes.s.tel)
      model$s.tel[i, 1:2] <<- s.cand
      model_lp_proposed <- model$calculate(calcNodes.s.tel)
      log_MH_ratio <- model_lp_proposed - model_lp_initial
      accept <- decide(log_MH_ratio)
      if(accept) {
        copy(from = model, to = mvSaved, row = 1, nodes = calcNodes.s.tel, logProb = TRUE)
      } else {
