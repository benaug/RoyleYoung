NimModel <- nimbleCode({
  ...
  #pairwise match-score means were absent in the non-pairwise model 
  #ZINB match-score parameters, q=1 same ID and q=2 different ID.
  for(q in 1:2){
    mu.match[q] ~ dunif(0,50) # unconditional mean
    p.zero.match[q] ~ dunif(0,1) # zero inflation
    theta.match[q] ~ dunif(0,500) # negative-binomial dispersion
  } 
  #--------------------------------------------------------------
  #Density model
  ...
  for(l1 in 1:(n.samples-1)){
    for(l2 in (l1+1):n.samples){
      ...
      scores[l1,l2] ~ dZINB(mu=mu.match[match.type[l1,l2]],
