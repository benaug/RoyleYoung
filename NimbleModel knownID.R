NimModel <- nimbleCode({
  ...
  for(i in 1:M){
    ...
    for(k in 1:K){
      ...
      y[i,k] ~ dRYmarg(u.cell=u.cell[i,k],sigma=sigma,z=z[i],
                       u.cell.survey=u.cell.survey[i,k],survey.map=survey.map[1:n.cells,k],
                       p=p[1:n.surveyed.cells[k],k],survey=survey[1:n.cells,k],
