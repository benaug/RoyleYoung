...
library(truncnorm)
library(coda)
source("sim.RY unknownID pairwise ZINB.R") 
source("Nimble Functions unknownID pairwise ZINB.R") 
source("init.data.RY.unknownID.R") 
source("NimbleModel unknownID pairwise ZINB.R") 
source("Nimble Functions unknownID pairwise ZINB.R") 
...
#ZINB match-score parameters, order is same ID/different ID. 
mu.match <- c(30,3) #unconditional means
theta.match <- c(10,10) #dispersion
p.zero.match <- c(0.1,0.1) #zero inflation
...
data <- sim.RY.pairwise.ZINB(D.beta0=D.beta0,D.beta1=D.beta1,rsf.beta=rsf.beta, 
               ...
               mu.match=mu.match,theta.match=theta.match,p.zero.match=p.zero.match) 
...
str(data$capture$scores) # upper-triangle matrix pairwise match scores
#visualize observed ZINB match scores using true simulated match type. 
matches <- data$capture$scores[data$truth$match.type==1] 
matches <- matches[!is.na(matches)] 
nonmatches <- data$capture$scores[data$truth$match.type==2] 
nonmatches <- nonmatches[!is.na(nonmatches)] 
...
#Pairwise score matrix is model data; diagonal is unused but set to 0 for a complete matrix. 
scores <- data$capture$scores 
diag(scores) <- 0 
...
Niminits <- list(z=nimbuild$z,N=nimbuild$N, #must init N to be sum(z.init)
                 ...
                 mu.match=rep(mean(scores,na.rm=TRUE),2), 
                 p.zero.match=c(0.1,0.1),theta.match=c(100,100), 
                 rsf.beta=2,
...
Nimdata <- list(y.true=matrix(NA,nrow=M,ncol=K),u=array(NA,dim=c(M,K,2)), 
                ...
                InSS=data$constants$InSS,z=nimbuild$z,rsf.cov=rsf.cov,scores=scores) 

#set parameters to monitor
parameters <- c('beta.p.int','beta.p.effort','rsf.beta','D.beta1','sigma','N','D0','lambda','n', 
                'mu.match','p.zero.match','theta.match') 
...
config.nodes <- c('beta.p.int','beta.p.effort',"sigma",'rsf.beta','mu.match','p.zero.match','theta.match')
...
#Pairwise stochastic score nodes are the upper triangle; build a map from each sample to all score nodes involving it. 
calcNodes.scores <- Rmodel$expandNodeNames("scores") 
calcNodes.match.type <- Rmodel$expandNodeNames("match.type") 
tmp.score <- matrix(NA,n.samples,n.samples) 
idx.score <- 1 
for(j in 2:n.samples){ 
  for(i in 1:(j-1)){ 
    tmp.score[i,j] <- idx.score 
    idx.score <- idx.score+1 
  } 
} 
map.score <- matrix(NA,n.samples,n.samples-1) 
map.score[1,] <- tmp.score[1,-1] 
for(i in 2:n.samples){ 
  tmp2 <- c(tmp.score[,i],tmp.score[i,]) 
  map.score[i,] <- tmp2[!is.na(tmp2)] 
} 
calcNodes.all <- unique(c(Rmodel$getDependencies("y.true"),Rmodel$getDependencies("u"),
                          calcNodes.match.type,calcNodes.scores)) 
...
data$truth$mu.match 
data$truth$p.zero.match 
data$truth$theta.match 
